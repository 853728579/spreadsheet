...
MainWindow::MainWindow()
{
    ...
    mdiArea->addSubWindow(spreadsheet);
    ...
    setWindowIcon(QIcon(":/images/text.png"));
...
#endif
    Spreadsheet *sp = new Spreadsheet(this);
    mdiArea->addSubWindow(sp);
}
...
void MainWindow::createActions()
{
    ...
    //使用快捷键Ctrl+N,  而在MAC OS X中则是command+N,
    //通过使用适当的QKeySequence::StandardKey枚举值，
    //就可以确保QT能够为应用程序在其运行的平台上提供正确的快捷键
    ...
    pasteAction->setStatusTip(tr("Paste the current clipboard's contents to the selection"));
    ...
    connect(selectRowAction, SIGNAL(triggered()),spreadsheet, SLOT(selectCurrenRow()));

    selectColumnAction = new QAction(tr("&Column"), this);
    selectColumnAction->setStatusTip(tr("Select all the cells in the current column"));
    connect(selectColumnAction, SIGNAL(triggered()), spreadsheet, SLOT(selectCurrenColumn()));
